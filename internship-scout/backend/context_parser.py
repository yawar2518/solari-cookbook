...

{
  ...
  "experience": {
    ...
    "location_scope": "string or null: one of [local, national, global] — local means their city/country only",
  ...
  "search_keywords": ["3-6 short keyword strings for job board searches"],
  "summary": "2-3 sentence human-readable summary of this person"
}

Rules:
- inferred_roles must have at least 2 and at most 5 roles
- search_keywords should be specific (e.g. 'Python backend intern' not just 'Python')
- if user mentions no role preference, infer purely from skills and context
- be generous with inferred skills — databases course means SQL, OOP course means Java or Python OOP
...
- if user says 'open to anything' or doesn't mention location → all location fields null except location_type: any"""
